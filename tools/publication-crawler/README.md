...
| `source_class` | Where the address was published: `organisation_own_site`, `public_register`, `third_party_directory`, or `aggregated_list`. Same-domain pages default to `organisation_own_site`; off-domain starts must supply `publication_source_class` or `address_source_class` in the input. |
...
| `evidence_record_version` | Evidence schema version. Current value: `2026-08-04.web452`. |

`source_class` deliberately uses four separate values because organisation-owned pages, public registers, third-party directories and aggregated lists are not interchangeable. Do not collapse them into a generic "published online" bucket at export time.
...
Operational gate: do not run production crawls from older checkpoints or older crawler versions. Production evidence rows must be generated by `evidence_record_version = 2026-08-04.web452` or later so every row has per-address provenance.
